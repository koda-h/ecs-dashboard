import type { ServicePermission, ViewPermission } from "@/lib/users/permission";
export type { ServicePermission, ViewPermission };
...
/** ViewPermission を部分上書きして生成する基本ファクトリ */
export function buildViewPermission(
  overrides: Partial<ViewPermission> = {}
): ViewPermission {
  return {
    clusterArn: `arn:aws:ecs:${REGION}:${ACCOUNT_ID}:cluster/test-cluster`,
    clusterName: "test-cluster",
...
/**
 * クラスター名とサービス名を指定して ViewPermission を生成する。
 * ARN もクラスター名・サービス名に合わせて生成される。
 */
export function buildViewPermissionInCluster(
  clusterName: string,
  serviceName: string
): ViewPermission {
  return buildViewPermission({
    clusterArn: `arn:aws:ecs:${REGION}:${ACCOUNT_ID}:cluster/${clusterName}`,
    clusterName,
    serviceArn: `arn:aws:ecs:${REGION}:${ACCOUNT_ID}:service/${clusterName}/${serviceName}`,
    serviceName,
